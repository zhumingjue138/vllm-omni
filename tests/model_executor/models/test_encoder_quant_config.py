...
For serialized ModelOpt and SVDQuant checkpoints the target model component is
the only quantized component. Vision and audio encoder weights are BF16 with no
serialized scale or correction tensors, so the same config must not be applied
to them.
...
def test_pre_quantized_returns_none(method: str) -> None:
    ...
    pre-quantized methods (ModelOpt variants and SVDQuant)."""
